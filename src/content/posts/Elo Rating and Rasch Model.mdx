...
---
## Context
When telling my spycholinguistic lecturer about my idea of using the Elo rating system to assess the difficulty of word and non-words items, alongside the testees themselves and avoid having to make preliminary studies, she told me about the Rasch model. I never heard of it before, and after a quick look to it, it seemed indeed, really similar to my idea.

## A Look at the Adjustment Mechanism in the Elo System
The Elo rating system can be best described as a self-fulfilling profecy. Based on the rating of to participents, the system gives a probability for the outcome of a match. If the profecy is fulfilled, it means that the ratings are correct and nothing changes, otherwise, the system gradually adjusts these ratings for the next time, and keeps doing so until reaching a point of equilibrium, that is, untill the profecy is fulfilled.

This correction is made by constantly adding to the current rating the difference of actual outcome and the expected result. If the expected output was correct, nothing changes. If 1 is expected (a win in chess) and 1 occures, then the rating does not change. If the rating difference underestimated the outcome, the rating of the winner grows, the one of the looser decrease. Here is another example when the ratings are equal. 
Often time, we want challenges where there is a 50% chance of success, so that the ratings keep evolving towards their "intrinsic value". In chess, we select players with aproximatively the same rating, so that their chances of winning are equal. If one player win, when the expected score was 50%-50%, then one player will increase their rating by $1-0.5=+0.5$ and the other by $0-0.5=-0.5$.

Still in chess, this difference is multiplied by a factor called a $K$ value, to speed up the adjustment process and make it so that less matches have to be played to find out the stable rating of the player. In the case of the **Rasch model**, the idea is exactly the same, exept that half of the participents are a task to accomplish and those tasks are given the same rating as the test takers in a way that, if test takers find the solution for tasks that most player fail at, their rating will increase faster than if they find the right answer to simpler questions (that is, a task that most test takers succeed at). The only difference between the two systems so far is absence of a $K$ value in the Rasch model, but as we saw, it is not a necessity neither.
...
The $400$ is used to spread the ratings between values of less than a hundred to several thousands, making the ratings more human readable. Without this "spreading fraction", the Elo rating of Magnus Carlsen would bearly reach $7.205$. Multiplying all the ratings by $400$, simply allows us avoid dealing with decimals, and makes the system "cleaner". This spreading is also the reason why we need a $K$ value when adjusting the ratings. Without this fraction, we don't need a $K$ value anymore, of maybe one between $0$ and $1$ in order to slow the progression of the ratings and limit their volativity.
...
$$\Leftrightarrow e^{R_A} \div e^{R_B}$$

$$\Leftrightarrow \frac{1}{e^{-R_A}} \times \frac{1}{e^{R_B}}$$

$$ \Leftrightarrow \frac{1}{e^{-R_A} \times e^{R_B}}$$

$$ \Leftrightarrow \frac{1}{e^{R_B-R_A}}$$
...
$$\Leftrightarrow \frac{1}{\frac{1+x}{x}}$$  


$$\Leftrightarrow \frac{1}{\frac{x}{x}+\frac{1}{x}}$$


$$\Leftrightarrow \frac{1}{1+\frac{1}{x}}$$

we can now state that:

$$\frac{1}{1+e^{R_B-R_A}}$$

$$\Leftrightarrow \frac{1}{1+\frac{1}{e^{R_A-R_B}}}$$ (following **Part 1**)


$$\Leftrightarrow \frac{1}{1+e^{R_B-R_A}}$$
(following **Part 2**)
...
Both systems where invended at the same period, that is, around 1960 with Elo's first publication on the matter in 1961 (Elo 1961) and Rasch's in 1960 (Rasch 1980, p. 197). But on inspection of the sources, none of them mentions the other's work, Elo claims he started working on the question in 1959 (Elo 1986, p. 4). Furthermore, their first publications come as the solutions to problems that have their own history, tracing back many years before these systems where implemented or published the first time. The truth is, those system may well have been invented independently from each other without nor Rasch nor Elo ever learning about the other's work...
...
Now, I wonder how much science or tooling from other fields scientists could successfully implement in their own domains. For example, marketing and language learning; what if language learning app relied on the same algorithms as those selecting the adds we see everyday? But to "feed" the learners with exactly the content they need to keep progressing or stay motivated... What about other fields of education?
